...
from applecider.preprocessing_utils.preprocess_multimodal import (
    Config,
...
def preprocess_data(raw_path, spec_path, output_path):
    ...
    compute_feature_stats_safe(
        cfg.output_root / "manifest_train.csv", "event", cfg.output_root
    )  # stats for the continuous light curve features
    compute_feature_stats_safe(
        cfg.output_root / "manifest_train.csv", "meta", cfg.output_root
    )  # stats for metadata columns
