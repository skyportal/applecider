...
class CNN_tower(nn.Module):
    def __init__(self, output_dims=512, img_size=49):
        ...
        self.attention = nn.Sequential(
            nn.Conv2d(128, 128, kernel_size=1), nn.ReLU(), nn.Conv2d(128, 1, kernel_size=1), nn.Sigmoid()
        )

        # Projection for channel 1 (no position/attention)
        self.proj_ch1 = nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Flatten(), nn.Linear(128, 128))
    ...
    def forward(self, x):
        ...
        # With:
        distance = torch.sqrt(
            (centroid_ch0[:, 0] - centroid_ch2[:, 0]) ** 2 + (centroid_ch0[:, 1] - centroid_ch2[:, 1]) ** 2
        )
...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, batch_size, height, width, device):
        ...
        pos_x = torch.stack((pos_x[:, 0::2].sin(), pos_x[:, 1::2].cos()), dim=2).flatten(
            1
        )  # (width, num_pos_feats//2)
        pos_y = torch.stack((pos_y[:, 0::2].sin(), pos_y[:, 1::2].cos()), dim=2).flatten(
            1
        )  # (height, num_pos_feats//2)
        ...
        fourier_input = torch.cat(
            [torch.sin(coord_map * 2 * math.pi), torch.cos(coord_map * 2 * math.pi)], dim=1
        )  # (B, 4, H, W)
        ...
        full_pos = torch.cat([pos, learned_pos, fourier_features], dim=1)  # (B, 2.5*C, H, W)
...
class SplitHeadConvNeXt(nn.Module):
    ...
    def __init__(self, pretrained=False, in_chans=4, outdims=4):
        ...
        self.head_aux = nn.Sequential(
            nn.LayerNorm(features), nn.Linear(features, outdims), nn.Tanh()
...
def plot_pr_curves(ax, probs, targets):
    ...
    for class_idx, class_name in enumerate(CLASSES):
        precision, recall, _ = precision_recall_curve(targets[:, class_idx], probs[:, class_idx])
        ax.plot(
            recall,
            precision,
            color=colors[class_idx % len(colors)],  # Auto-scale to class count
            lw=2,
            label=f"{class_name} (AUC={auc(recall, precision):.2f}",
        )  # Show AUC in legend
...
def plot_combined_results(loader, model, DEVICE, seed=None, best_model_path=None):
    ...
    plt.tight_layout()
    model_name = (
        model.__class__.__name__ if hasattr(model, "__class__") else model.split("/")[-1].replace(".pth", "")
    )
    filename = (
        f"/projects/bcrv/abrown3/jobs/models_img_meta/{model_name}_{seed}_{wandb.run.id}.png"
        if seed
        else f"/projects/bcrv/abrown3/jobs/models_img_meta/{model_name}_{wandb.run.id}.png"
    )
...
def print_metrics(loader, model, DEVICE):
    ...
    # PR-AUC
    pr_auc_mean, pr_aucs, fusion_loads, classification_loads = calculate_pr_auc_print_metrics(
        loader, model, DEVICE
    )
...
def main(config, trial):
    ...
    for run in range(1):
        ...
        model = XastroMiNN(
            ...
            num_mlp_experts=config["num_experts"],
        ...
        if model.__class__.__name__ == "XastroMiNN":
            optimizer = torch.optim.AdamW(
                [
                    ...
                    {
                        "params": model.image_tower.parameters(),
                        "weight_decay": config["cnn_decay"],
                        "lr": LR * config["cnn_lr"],
                    },
                    # Metadata towers - higher regularization (more prone to overfitting)
                    {
                        "params": model.psf_tower.parameters(),
                        "weight_decay": config["psf_decay"],
                        "lr": LR * config["psf_lr"],
                    },
                    {
                        "params": model.lc_tower.parameters(),
                        "weight_decay": config["lc_decay"],
                        "lr": LR * config["lc_lr"],
                    },
                    {
                        "params": model.mag_tower.parameters(),
                        "weight_decay": config["mag_decay"],
                        "lr": LR * config["mag_lr"],
                    },
                    {
                        "params": model.spatial_tower.parameters(),
                        "weight_decay": config["spatial_decay"],
                        "lr": LR * config["spatial_lr"],
                    },
                    {
                        "params": model.coord_tower.parameters(),
                        "weight_decay": config["nst1_decay"],
                        "lr": LR * config["nst1_lr"],
                    },
                    {
                        "params": model.nst1_tower.parameters(),
                        "weight_decay": config["nst1_decay"],
                        "lr": LR * config["nst1_lr"],
                    },
                    {
                        "params": model.nst2_tower.parameters(),
                        "weight_decay": config["nst2_decay"],
                        "lr": LR * config["nst2_lr"],
                    },
                    # Currently using mega tower as the router essentially
                    {
                        "params": model.mega_tower.parameters(),
                        "weight_decay": config["lc_decay"],
                        "lr": LR * config["lc_lr"],
                    },
                    # Fusion components
                ...
                eps=config["eps"],
            )  # Avoids divison by zero errors
            print(f"using {model.__class__.__name__} model with test optimizing parameters")
        else:
            optimizer = torch.optim.AdamW(
                params=model.parameters(), lr=LR, betas=(config["beta1"], config["beta2"]), eps=config["eps"]
            )
        ...
        train_loader = get_data_loader(
            npy_dir="/work/nvme/bcrv/abrown3/preprocessed_data/data_multi/day10/",
            alert_file_list=Train["file"].to_list(),
            batch_size=32,
        ...
        val_loader = get_data_loader(
            ...
            alert_file_list=Val["file"].to_list(),
            batch_size=32,
            random_alert_per_epoch=False,
        )
        ...
        test_loader = get_data_loader(
            ...
            alert_file_list=Test["file"].to_list(),
            batch_size=32,
            random_alert_per_epoch=False,
        )
        ...
        coarse_weights = torch.tensor(
            [numberator / (int(count) ** (weight_exponent)) for idx, count in enumerate(class_counts)],
            device=device,
            dtype=torch.float32,
        )
        ...
        if config["criterion"].lower() == "focalexpert":
            criterion = FocalLossWithExpertSpecialization(
                ...
                gamma=config["gamma"],
                expert_weight=0.5,  # Add this to your config
            )
        ...
        for epoch in range(EPOCHS):
            ...
            if val_pr_auc_mean > best_pr_auc:
                ...
                torch.save(
                    model.state_dict(),
                    f"/projects/bcrv/abrown3/jobs/models_img_meta/best_model{seed}_{wandb.run.id}.pth",
                )
                model_best = copy.deepcopy(
                    model
                )  # there's definitely a better way to do this but I don't wanna look it up
        ...
        print("Random Baseline PR-AUCs (mean ± std):")
...
def random_baseline_pr_auc(loader, n_trials=1000):
    ...
    for trial in range(n_trials):
        ...
        for class_idx in range(num_classes):
            precision, recall, _ = precision_recall_curve(
                (targets == class_idx).astype(int), random_probs[:, class_idx]
            )
...
class ResidualTowerBlock(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        super().__init__()
        self.start_path = nn.Sequential(nn.Linear(input_dim, hidden_dim), nn.GELU())

        self.main_path = nn.Sequential(
            nn.LayerNorm(hidden_dim), nn.Dropout(0.25), nn.Linear(hidden_dim, output_dim)
        )

        self.activation = nn.Sequential(
            nn.LayerNorm(hidden_dim), nn.Dropout(0.25), nn.Linear(hidden_dim, output_dim), nn.Sigmoid()
        )
