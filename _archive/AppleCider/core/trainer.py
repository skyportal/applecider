...
class Trainer:
    ...
    def train_epoch(self, train_dataloader):
        ...
        for photometry, photometry_mask, images, metadata, spectra, labels in tqdm(train_dataloader, total=len(train_dataloader), desc='Train', colour='#9ACD32',leave=True):
            photometry, photometry_mask = photometry.to(self.device), photometry_mask.to(self.device)
            images, metadata, spectra = images.to(self.device),  metadata.to(self.device), spectra.to(self.device)
    ...
    def val_epoch(self, val_dataloader):
        ...
        with torch.no_grad():
            for photometry, photometry_mask, images, metadata, spectra, labels in tqdm(val_dataloader, total=len(val_dataloader), desc='Validation', colour='#9ACD32', leave=True):
                photometry, photometry_mask = photometry.to(self.device), photometry_mask.to(self.device)
                images, metadata, spectra = images.to(self.device), metadata.to(self.device), spectra.to(self.device)
                labels = labels.to(self.device)
