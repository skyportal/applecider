...
class SpectraProcessor:
    ...
    def preprocess_spectra(spectra):
        ...
        f = interp1d(
            spectra[:, 0], spectra[:, 1], kind="linear", bounds_error=False, fill_value="extrapolate"
        )
...
class DataSorter:
    ...
    def create_df_of_object_alerts_in_dataset(test_df, train_df, test_data_dir, train_data_dir):
        ...
        test_data = test_data.merge(
            test_df[["obj_id", "type"]], left_on="name", right_on="obj_id", how="left"
        )
        ...
        train_data = train_data.merge(
            train_df[["obj_id", "type"]], left_on="name", right_on="obj_id", how="left"
        )
    ...
    def split_and_compute_class_weights(
        df,
        step,
        max_samples,
        file_suffix=None,
        class_label_str=False,
        group_labels=False,
        group_labels_SN=False,
        save_files=False,
        save_path=None,
        split_ratio=0.8,
        random_seed=42,
        ...
        if os.path.isfile(train_file_path):
            ...
            if os.path.isfile(class_weights_path):
                try:
                    ...
                    if class_label_str and [type(k) for k in class_weight_dict.keys()][0] == np.int64:
                        class_weight_dict = dict(
                            (v, class_weight_dict.get(k, k)) for (k, v) in id2target.items()
                        )
                ...
                class_weights = compute_class_weight(
                    class_weight="balanced", classes=unique_labels, y=train_df[step]
                )
            ...
            print("Files do not exist, create files!")
