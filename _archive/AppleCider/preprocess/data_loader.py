...
def create_data_loaders(config):
    ...
    train_dataset = SpectraPTDataset(
        pt_path=config["train_dir"], redshift_key="labels", class_order=class_order, apply_random_mask=True
    )
    val_dataset = SpectraPTDataset(
        pt_path=config["val_dir"], redshift_key="labels", class_order=class_order, apply_random_mask=False
    )
    test_dataset = SpectraPTDataset(
        pt_path=config["test_dir"], redshift_key="labels", class_order=class_order, apply_random_mask=False
    )
...
def create_fusion_loaders_classification(config):
    ...
    train_set = FusionClassificationDataset(pt_file=config["train_dir"], class_order=class_order)
    val_set = FusionClassificationDataset(pt_file=config["val_dir"], label_to_index=train_set.label_to_index)
    test_set = FusionClassificationDataset(
        pt_file=config["test_dir"], label_to_index=train_set.label_to_index
    )
