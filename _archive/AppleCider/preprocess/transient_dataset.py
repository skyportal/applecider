...
class TransientDataset:
    ...
    def preprocess_data(self, df_bts, base_path, max_mjd):
        ...
        for idx, row in tqdm(df_bts.iterrows(), total=df_bts.shape[0], desc="Loading data", leave=True):
            try:
                ...
                photo_df, alert_df, images = (
                    PhotometryProcessor.process_csv(obj_id, df_bts, base_path),
                    *AlertProcessor.get_process_alerts(obj_id, base_path),
                )
                ...
                for i in alert_indices:
                    ...
                    if len(photo_ready) <= 1:
                        print(
                            f"{obj_id} Failed photometry requirement after time cuts. Skip {obj_id} at index {i}!"
                        )
                        continue
    ...
    def process_and_save_sample(args):
        ...
        res_df = pd.DataFrame()
        photometry = sample["photometry"].pivot_table(
            index=["mjd"], columns="filter", values=["flux", "flux_error"]
        )
        photometry = photometry.reset_index()
        photometry.columns = [
            col[0] if col[0] == "mjd" else "_".join(col).strip() for col in photometry.columns.values
        ]
    ...
    def preprocess_and_save(self):
        ...
        args = [
            (
                ...
                self.include_spectra,
                self.include_flux_err,
                self.normalize_light_curve,
            )
        ...
        with multiprocessing.Pool(num_workers) as pool:
            list(
                tqdm(
                    pool.imap(TransientDataset.process_and_save_sample, args),
                    total=len(self.data_preprocess),
                    desc="Saving",
                    leave=True,
                )
            )
