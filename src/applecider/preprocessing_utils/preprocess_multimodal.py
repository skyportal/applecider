...
def load_photometry_obj(obj_id: str, data_dir: Path) -> pd.DataFrame:
    ...
    if len(csv_df) == 0 and len(al_df) == 0:
        return pd.DataFrame(
            columns=["obj_id", "jd", "mjd", "mag", "magerr", "flux", "flux_error", "fid", "filter"]
        )
...
def build_event_features(merged: pd.DataFrame) -> pd.DataFrame:
    ...
    g2r = pd.merge_asof(
        g.sort_values("mjd"), r.sort_values("mjd"), on="mjd", direction="nearest", tolerance=TOL
    )
    r2i = pd.merge_asof(
        r.sort_values("mjd"), i.sort_values("mjd"), on="mjd", direction="nearest", tolerance=TOL
    )
...
# ----------------------------
CTX_GLOBAL_KEYS = ["days_since_peak", "days_to_peak", "peakmag_so_far", "maxmag_so_far"]
...
class AlertIndex:
    ...
    def __init__(self, alerts: List[dict], require_all_3: bool = True):
        ...
        for fid in FIDS:
            ...
            self.by_fid[fid] = {"jd": np.array([float(a["_jd"]) for a in aa], dtype=np.float64), "alerts": aa}
    ...
    def get_best_in_window_by_sig(
        self, fid: int, jd: float, tol_days: float
...
def build_multimodal_for_object(
    obj_id: str, label_int: int, label_str: Optional[str], out_dir: Path, cfg: Config
) -> Optional[Dict]:
    ...
    for _, row in events.iterrows():
        ...
        if pick is None and last_choice.get(fid) is not None:
            ...
            if pick is None and cfg.allow_fallback_nearest_any:
                ...
                if near is not None:
                    ...
                    if (cfg.max_nearest_any_dt_days is not None) and (
                        abs(dt_days) > cfg.max_nearest_any_dt_days
                    ):
                        near = None
                if near is not None:
        ...
        extra = {
            ...
            if (not np.isnan(ctx_g["peakmag_so_far"]) and ctx_g["peakmag_so_far"] != 0)
            else np.nan,
            **ctx_c,
        }
        extra_keys = list(extra.keys())
        extra_vals = [
            (-999.0 if (v is None or (isinstance(v, float) and np.isnan(v))) else float(v))
...
def compute_feature_stats_safe(manifest_csv: Path, kind: str, out_dir: Path):
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    np.savez(
        out_dir / f"feature_stats_{kind}.npz",
...
def build_all_preprocessed(cfg: Config) -> pd.DataFrame:
    ...
    recs = []
    for oid, typ, lab in tqdm(
        spec_avail[["object_id", "type", "label_int"]].itertuples(index=False),
        total=len(spec_avail),
        desc="Building multimodal",
    ):
        try:
...
def make_splits_from_manifest(
    ...
    *,
    ...
    print(
        f"Keeping {len(keep_classes)} classes with ≥{min_per_class} examples "
    ...
    if can_strat:
        tr, temp = train_test_split(ids, train_size=train_frac, stratify=labs, random_state=seed)
        ...
        if not np.isclose(val_share + te_share, 1.0):
            val_share = 0.5
            te_share = 0.5
        va, te = train_test_split(temp, train_size=val_share, stratify=labs_temp, random_state=seed)
    else:
        if strict_stratify:
            raise ValueError(
                "Stratified split not feasible with current min_per_class; "
