...
class ImageAndMetadataDataset(HyraxDataset, OversamplerMixin):
    def __init__(self, config, data_location):
        ...
        self.raw_files = [
            np.load(os.path.join(data_location, file_name), allow_pickle=True).item()
        ...
        if self.use_oversampling:
            self.prepare_over_sampling(self.dataset_config["class_distribution"], self.class_at_index)
    ...
    def get_image(self, index):
        ...
        if self.enable_cache and index in self.image_cache:
            ...
            elif self.dataset_config["cutout_size"] != 63:
